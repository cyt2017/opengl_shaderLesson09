...
bool QGLESWIDGET::init_QGW(std::vector<QString> fileName)
{
    ...
    _box.initDataBox(CELL::float3(1.0f,1.0f,1.0f));
    for(int x=0;x<100;x+=5)
    {
        for(int z=0;z<100;z+=5)
        {
            CELL::matrix4 mat;
            mat.translate(CELL::float3(x,3,z));
            _matLoc.push_back(mat);
        }
    }
...
void QGLESWIDGET::render()
{
    ...
    _shader.begin();
    {
        glUniformMatrix4fv(_shader._matMV, 1, GL_FALSE, matMV.data());
        glUniformMatrix4fv(_shader._MVP, 1, GL_FALSE, matMVP.data());
        glUniform4f(_shader._fogColor,1.0f,0.0f,0.0f,1.0f);
        glUniform1f(_shader._fogDensity,0.05f);
        glUniform1f(_shader._fogR,50);
        ...
        for(int i=0;i<_matLoc.size();i++)
        {
            CELL::matrix4   mat = _matLoc[i];
            CELL::matrix4   mvp = matMVP*mat;
            CELL::matrix4   mv  = matMV *mat;
            glUniformMatrix4fv(_shader._MVP,1,false,mvp.data());
            glUniformMatrix4fv(_shader._matMV,1,GL_FALSE, mv.data());
